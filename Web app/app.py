...
app.secret_key = "mysecretkey"  # change this to a secure key in production
...
def add():
    ...
    if request.method == "POST":
        ...
        if name in namelist:
            return render_template("add.html", error = "Item already in your inventory", lis = sorted(lis))
        else :
            conn = sqlite3.connect("database.db")
            c = conn.cursor()
        ...
        lis = c.fetchall()
        conn.close()
        return render_template("add.html", lis = sorted(lis))
